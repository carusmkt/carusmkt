...
2. Open the deployed `admin.html`, enter the PIN, paste the token under **GitHub fine-grained token**, and click **Conectar token**. The token stays in the current tab's JavaScript memory and is never included in the site files, posts, browser storage, or a URL. Re-enter it after reloading the page.
